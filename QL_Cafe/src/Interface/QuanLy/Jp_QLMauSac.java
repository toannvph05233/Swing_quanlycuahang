 ...
 * @author ACER
 */
public class Jp_QLMauSac extends javax.swing.JPanel {
    ConnectSQL cn = new ConnectSQL();
    /**
     * Creates new form Jp_QLMauSac
     */
    public static Jp_QLMauSac mausac;
    public Jp_QLMauSac() {
        initComponents();
        mausac = this;
        FillTable();
    ...
    public void FillTable() {
        ArrayList<MauSac> arrTable = cn.GetMauSac();
        DefaultTableModel tbmodel = new DefaultTableModel();

        tbmodel.addColumn("Mã màu");
        tbmodel.addColumn("Màu sắc");
        

        if (arrTable != null) {
            int somausac = 0;
            for (MauSac l : arrTable) {
                somausac++;
                tbmodel.addRow(new Object[]{l.GetMaMauSac(), l.GetTenMauSac()});
            }
            lblthongtin.setText(String.valueOf(somausac)+" màu sắc");
        } else {
            JOptionPane.showMessageDialog(null, "Không có màu nào");
        }
        tbMauSac.setModel(tbmodel);
        for(int i = 0; i < tbMauSac.getColumnCount();i++){
            Class<?> col = tbMauSac.getColumnClass(i);
            tbMauSac.setDefaultEditor(col, null);
        }        
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Tổng số màu sắc:");

        jLabel1.setFont(new java.awt.Font("Tahoma", 1, 12)); // NOI18N
        jLabel1.setText("Tìm màu:");
    ...
    private void bntXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntXoaActionPerformed
        int[] selectedRows = tbMauSac.getSelectedRows();

        if (selectedRows.length <= 0) {
            JOptionPane.showMessageDialog(null, "Bạn chưa chọn màu nào !");
        } else {
            ArrayList<String> listmamausac = new ArrayList<String>();
            String sp = "";
            for (int i : selectedRows) {
                String ma = (String) tbMauSac.getValueAt(i, 0);
                listmamausac.add(ma);
                String tenmau = (String) tbMauSac.getValueAt(i, 1);

                sp += tenmau + "\n";
            }
            int qs;
            qs = JOptionPane.showConfirmDialog(null, "Xóa màu: \n " + sp, "Xóa màu", JOptionPane.YES_NO_OPTION);
            if (qs == JOptionPane.YES_OPTION) {
                boolean xoa = cn.DeleteMauSac(listmamausac);
                ...
                JOptionPane.showMessageDialog(null, "Không xóa được màu !");
    ...
    private void txttimKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txttimKeyReleased
        ArrayList<MauSac> arrTable = cn.SearchMauSac(txttim.getText());
        if(arrTable != null){
            DefaultTableModel tbmodel = new DefaultTableModel();
            ...
            int sohang = 0;
            for (MauSac b : arrTable) {
                sohang++;
                tbmodel.addRow(new Object[]{b.GetMaMauSac(), b.GetTenMauSac()});
            }
            tbMauSac.setModel(tbmodel);
            for(int i = 0; i < tbMauSac.getColumnCount();i++){
                Class<?> col = tbMauSac.getColumnClass(i);
    ...
    private void bntThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntThemActionPerformed
        DLQl_Them_Mau_Sac b = new DLQl_Them_Mau_Sac(Run.QlCafe, true);
        b.setVisible(true);
    ...
    private void bntSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntSuaActionPerformed
        int select=tbMauSac.getSelectedRow();
        if(select<0){
            JOptionPane.showMessageDialog(null, "Bạn chưa chọn màu nào !");
        }else{
            String mamausac = (String) tbMauSac.getValueAt(select, 0);

            DLQl_Sua_Mau_Sac sua = new DLQl_Sua_Mau_Sac(Run.QlCafe, true, mamausac);
            sua.setVisible(true);
        }
