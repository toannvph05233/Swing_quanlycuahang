 ...
 * @author ACER
 */
public class Jp_QLKieuDang extends javax.swing.JPanel {
    ConnectSQL cn = new ConnectSQL();
    /**
     * Creates new form Jp_QLKieuDang
     */
    public static Jp_QLKieuDang kieudang;
    public Jp_QLKieuDang() {
        initComponents();
        kieudang = this;
        FillTable();
    }
    public void FillTable() {
        ArrayList<KieuDang> arrTable = cn.GetKieuDang();
        ...
        if (arrTable != null) {
            int sokieudang = 0;
            for (KieuDang l : arrTable) {
                sokieudang++;
                tbmodel.addRow(new Object[]{l.GetMaKieuDang(), l.GetTenKieuDang()});
            }
            lblthongtin.setText(String.valueOf(sokieudang)+" kiểu dáng");
        } else {
            JOptionPane.showMessageDialog(null, "Không có kiểu nào");
        }
        tbKieuDang.setModel(tbmodel);
        for(int i = 0; i < tbKieuDang.getColumnCount();i++){
            Class<?> col = tbKieuDang.getColumnClass(i);
            tbKieuDang.setDefaultEditor(col, null);
        }        
    }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Tổng số kiểu dáng:");
    ...
    private void bntXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntXoaActionPerformed
        int[] selectedRows = tbKieuDang.getSelectedRows();

        if (selectedRows.length <= 0) {
            JOptionPane.showMessageDialog(null, "Bạn chưa chọn kiểu nào !");
        } else {
            ArrayList<String> listmakieudang = new ArrayList<String>();
            String sp = "";
            for (int i : selectedRows) {
                String ma = (String) tbKieuDang.getValueAt(i, 0);
                listmakieudang.add(ma);
                String tenkieu = (String) tbKieuDang.getValueAt(i, 1);
            ...
            if (qs == JOptionPane.YES_OPTION) {
                boolean xoa = cn.DeleteKieuDang(listmakieudang);
                if (xoa == true) {
                    FillTable();
                    try{
                ...
                JOptionPane.showMessageDialog(null, "Không xóa được kiểu dáng !");
    ...
    private void txttimKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txttimKeyReleased
        ArrayList<KieuDang> arrTable = cn.SearchKieuDang(txttim.getText());
        if(arrTable != null){
            DefaultTableModel tbmodel = new DefaultTableModel();

            tbmodel.addColumn("Mã kiểu dáng");
            tbmodel.addColumn("Tên kiểu dáng");

            int sohang = 0;
            for (KieuDang b : arrTable) {
                sohang++;
                tbmodel.addRow(new Object[]{b.GetMaKieuDang(), b.GetTenKieuDang()});
            }
            tbKieuDang.setModel(tbmodel);
            for(int i = 0; i < tbKieuDang.getColumnCount();i++){
                Class<?> col = tbKieuDang.getColumnClass(i);
    ...
    private void bntThemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntThemActionPerformed
        DLQl_Them_Kieu_Dang b = new DLQl_Them_Kieu_Dang(Run.QlCafe, true);
        b.setVisible(true);
    ...
    private void bntSuaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bntSuaActionPerformed
        int select=tbKieuDang.getSelectedRow();
        if(select<0){
            JOptionPane.showMessageDialog(null, "Bạn chưa chọn kiểu nào !");
        }else{
            String makieudang = (String) tbKieuDang.getValueAt(select, 0);

            DLQl_Sua_Kieu_Dang sua = new DLQl_Sua_Kieu_Dang(Run.QlCafe, true, makieudang);
            sua.setVisible(true);
        }
